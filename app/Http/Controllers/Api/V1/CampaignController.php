...
class CampaignController extends Controller
{
    public function get_basic_campaigns(Request $request){
        ...
        try {
            $campaigns = Campaign::whereHas('stores', function($query)use($zone_id){
    ...
    public function basic_campaign_details(Request $request){
        ...
        try {
            $campaign = Campaign::with(['stores'=>function($q)use($zone_id){
                $q->withOpen()->where('zone_id', $zone_id);
            }])
            ->when(config('module.current_module_id'), function($query){
                $query->module(config('module.current_module_id'));
            })
    ...
    public function get_item_campaigns(Request $request){
        ...
        $zone_id= $request->header('zoneId');
        try {
            $campaigns = ItemCampaign::active()->whereHas('store', function($query)use($zone_id){
                $query->where('zone_id', $zone_id);
            })
            ->when(config('module.current_module_id'), function($query){
                $query->module(config('module.current_module_id'));
