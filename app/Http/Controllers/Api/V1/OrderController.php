    ...
    public function place_order(Request $request)
    {
        ...
        if($request->order_type !== 'parcel')
        {
            ...
            if(!$store)
            {
                return response()->json([
                    'errors' => [
                        ['code' => 'order_time', 'message' => trans('messages.store_not_found')]
                    ]
                ], 404);
            }
            ...
            if($store->open == false)
            {
                return response()->json([
                    'errors' => [
                        ['code' => 'order_time', 'message' => trans('messages.store_is_closed_at_order_time')]
                    ]
                ], 406);
            }
        ...
        if($request->latitude && $request->longitude)
        {
            ...
            $zone = Zone::where('id', $request->header('zoneId'))->contains('coordinates', $point)->first();
            if(!$zone)
            {
                $errors = [];
        ...
        $order->zone_id = isset($store)?$store->zone_id:$request->header('zoneId', null);
        ...
        if($request->order_type !== 'parcel')
        {
                    ...
                    if ($product) {
                        ...
                        if(config('module.'.$product->module->module_type)['stock'])
                        {
                            if($c['quantity']>$stock)
                            {
                                return response()->json([
                                    'errors' => [
                                        ['code' => 'campaign', 'message' => trans('messages.product_out_of_stock_warning',['item'=>$product->title])]
                                    ]
                                ], 406);
                            }
                    ...
                    if ($product) {
                        ...
                        if(config('module.'.$product->module->module_type)['stock'])
                        {
                            if($c['quantity']>$stock)
                            {
                                return response()->json([
                                    'errors' => [
                                        ['code' => 'campaign', 'message' => trans('messages.product_out_of_stock_warning',['item'=>$product->name])]
                                    ]
                                ], 406);
                            }
                        ...
                        return response()->json([
                            'errors' => [
                                ['code' => 'item', 'message' => trans('messages.product_unavailable_warning')]
                            ]
                        ], 404);
                    }
                }
            ...
            if($store->minimum_order > $product_price + $total_addon_price )
            {
                return response()->json([
                    'errors' => [
                        ['code' => 'order_time', 'message' => trans('messages.you_need_to_order_at_least', ['amount'=>$store->minimum_order.' '.Helpers::currency_code()])]
                    ]
                ], 406);
            }
        ...
        else
        {
            ...
            {
                $errors = [];
                array_push($errors, ['code' => 'receiver_details', 'message' => trans('messages.out_of_coverage')]);
                return response()->json([
                    'errors' => $errors
                ], 403);
            }
