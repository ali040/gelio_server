...
class StoreLogic
{
    public static function get_stores( $zone_id, $filter, $type, $limit = 10, $offset = 1, $featured=false)
    {
        $paginator = Store::
        withOpen()
    ...
    public static function get_latest_stores($zone_id, $limit = 10, $offset = 1, $type='all')
    {
        $paginator = Store::withOpen()
        ->with(['discount'=>function($q){
            return $q->validate();
        }])->where('zone_id', $zone_id)
        ->when(config('module.current_module_id'), function($query){
    ...
    public static function get_popular_stores($zone_id, $limit = 10, $offset = 1, $type = 'all')
    {
        $paginator = Store::withOpen()
        ->with(['discount'=>function($q){
            return $q->validate();
    ...
    public static function search_stores($name, $zone_id, $category_id= null,$limit = 10, $offset = 1, $type = 'all')
    {
        $key = explode(' ', $name);
        $paginator = Store::withOpen()->with(['discount'=>function($q){
            return $q->validate();
