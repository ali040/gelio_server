...
class ProductLogic
{
    ...
    {
        $key = explode(' ', $name);
        $paginator = Item::active()->whereHas('store', function($q)use($zone_id){
    ...
    public static function popular_products($zone_id, $limit = null, $offset = null, $type = 'all')
    {
        ...
        {
            $paginator = Item::whereHas('store', function($q)use($zone_id){
                $q->where('zone_id', $zone_id)->Weekday();
            })
            ->when(config('module.current_module_id'), function($query){
                $query->module(config('module.current_module_id'));
        ...
        ->type($type)->whereHas('store', function($q)use($zone_id){
            $q->where('zone_id', $zone_id)->Weekday();
    ...
    public static function most_reviewed_products($zone_id, $limit = null, $offset = null, $type = 'all')
    {
        ...
        $paginator = Item::active()->type($type)->whereHas('store', function($q)use($zone_id){
            $q->where('zone_id', $zone_id)->Weekday();
        })
        ->when(config('module.current_module_id'), function($query){
            $query->module(config('module.current_module_id'));
