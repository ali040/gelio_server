        ...
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col-sm-4 col-12">
                    <h1 class="page-header-title"> {{__('messages.item')}} {{__('messages.list')}}<span class="badge badge-soft-dark ml-2" id="foodCount">{{$items->total()}}</span></h1>
                </div>
                ...
                <div class="col-sm-4 col-6">
                    <select name="store_id" id="store" onchange="set_store_filter('{{url()->full()}}',this.value)" data-placeholder="{{__('messages.select')}} {{__('messages.store')}}" class="js-data-example-ajax form-control" onchange="getStoreData('{{url('/')}}/admin/vendor/get-addons?data[]=0&store_id=',this.value,'add_on')" required title="Select Store" oninvalid="this.setCustomValidity('{{__('messages.please_select_store')}}')">
                    @if($store)    
                        ...
                        <table id="datatable" class="table table-borderless table-thead-bordered table-nowrap table-align-middle card-table"
                            ...
                            @foreach($items as $key=>$item)
                                <tr>
                                    ...
                                    <td>
                                    {{Str::limit($item->category?$item->category->name:__('messages.category_deleted'),20,'...')}}
                                    </td>
                                    <td>
                                    {{Str::limit($item->store?$item->store->name:__('messages.store deleted!'), 20, '...')}}
                                    </td>
