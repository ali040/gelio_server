...
@section('title',__('messages.edit').' '.__('messages.account_transaction'))
                ...
                <div class="row">
                    <div class="col-md-4">
                        ...
                        <label class="input-label" for="type">{{__('messages.type')}}<span class="input-label-secondary"></span></label>
                            <select name="type" id="type" class="form-control">
                                <option value="deliveryman" {{$account_transaction->from_type=='deliveryman'?'selected':''}}>{{__('messages.deliveryman')}}</option>
                                <option value="restaurant" {{$account_transaction->from_type=='deliveryman'?'selected':''}}>{{__('messages.store')}}</option>
                            </select>
                        </div>  
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            <label class="input-label" for="restaurant">{{__('messages.store')}}<span class="input-label-secondary"></span></label>
                            <select id="restaurant" name="store_id" data-placeholder="{{__('messages.select')}} {{__('messages.store')}}" class="form-control" title="Select Restaurant" {{$account_transaction->deliveryman?'disabled':''}}>
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <select id="deliveryman" name="deliveryman_id" data-placeholder="{{__('messages.select')}} {{__('messages.deliveryman')}}" class="form-control" title="Select deliveryman" {{$account_transaction->restaurant?'disabled':''}}>
        ...
        $.post({
            ...
            success: function (data) {
                if (data.errors) {
                    ...
                    toastr.success('{{__('messages.transaction_updated')}}', {
                        CloseButton: true,
                        ProgressBar: true
                    });
                    setTimeout(function () {
