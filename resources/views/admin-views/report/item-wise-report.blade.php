...
@section('title',__('messages.item_wise_report'))
        ...
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col-12 mb-2 mb-sm-0">
                    <h1 class="page-header-title"><i class="tio-filter-list"></i> {{__('messages.item_wise_report')}} <span class="h6 badge badge-soft-success ml-2" id="itemCount">( {{session('from_date')}} - {{session('to_date')}} )</span></h1>
                </div>
            </div>
            <div class="row">
                ...
                <div class="col-sm-4 col-12">
                    <select name="store_id" onchange="set_store_filter('{{url()->full()}}',this.value)" data-placeholder="{{__('messages.select')}} {{__('messages.store')}}" class="js-data-example-ajax form-control">
                        @if(isset($store))    
                        <option value="{{$store->id}}" selected>{{$store->name}}</option>
                        @else
                        <option value="all" selected>{{__('messages.all')}} {{__('messages.stores')}}</option>
                        @endif
                    </select>
                </div>
            </div>
                    ...
                    @foreach($items as $key=>$item)
                        <tr>
                            ...
                            <td>
                                ...
                                @else
                                {{__('messages.store')}} {{__('messages.deleted')}}
                                @endif
                            </td>
                            <td>
                                @if($item->store)
