...
@section('title',__('messages.update_parcel_category'))
        ...
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col-sm mb-2 mb-sm-0">
                    <h1 class="page-header-title"><i class="tio-edit"></i>{{__('messages.update_parcel_category')}}</h1>
                </div>
            </div>
        </div>
        <!-- End Page Header -->
                ...
                <form action="{{route('admin.parcel.category.update',[$parcel_category['id']])}}" method="post" enctype="multipart/form-data">
                        ...
                        @foreach(json_decode($language) as $lang)
                            ...
                            <div class="card p-4 {{$lang != $default_lang ? 'd-none':''}} lang_form" id="{{$lang}}-form">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="name[]" id="{{$lang}}_name" class="form-control" placeholder="{{__('messages.new_food')}}" value="{{$translate[$lang]['name']??$parcel_category['name']}}" {{$lang == $default_lang? 'required':''}} oninvalid="document.getElementById('en-link').click()">
                                </div>
                    ...
                    <div class="card p-4" id="{{$default_lang}}-form">
                        <div class="form-group">
                            ...
                            <input type="text" name="name[]" class="form-control" placeholder="{{__('messages.new_food')}}" value="{{$parcel_category['name']}}" required>
                        </div>
