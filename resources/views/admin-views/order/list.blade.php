    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <div class="row align-items-center mb-3">
                <div class="col-9">
                    <h1 class="page-header-title text-capitalize">@if ($parcel_order) {{__('messages.parcel')}} {{__('messages.orders')}} @else {{str_replace('_',' ',$status)}} {{__('messages.orders')}} @endif <span class="badge badge-soft-dark ml-2">{{$total}}</span></h1>
                </div>
                <div class="col-3">
                    @if (!$parcel_order)
                        ...
                        <div class="d-sm-flex justify-content-sm-end align-items-sm-center">
                            ...
                            <div class="hs-unfold mr-2">
                                ...
                                <div id="usersExportDropdown"
                                    ...
                                    <a id="export-copy" class="dropdown-item" href="javascript:;" title="{{__('messages.current_page_only')}}">
                                        <img class="avatar avatar-xss avatar-4by3 mr-2"
                                             src="{{asset('public/assets/admin')}}/svg/illustrations/copy.svg"
                                             alt="Image Description">
                                        {{__('messages.copy')}}
                                    </a>
                                    <a id="export-print" class="dropdown-item" href="javascript:;" title="{{__('messages.current_page_only')}}">
                                        <img class="avatar avatar-xss avatar-4by3 mr-2"
                                ...
                                <div id="showHideDropdown"
                                    ...
                                    <div class="card card-sm">
                                        <div class="card-body">
                                            ...
                                            <div class="d-flex justify-content-between align-items-center mb-3">
                                                <span class="mr-2">{{$parcel_order?__('messages.parcel_category'):__('messages.store')}}</span>
            ...
            <div class="table-responsive datatable-custom">
                <table id="datatable"
                    ...
                    <tr>
                        ...
                        @if ($parcel_order)
                            <th>{{__('messages.parcel_category')}}</th>
                        @else
                    ...
                    @foreach($orders as $key=>$order)
                        ...
                        <tr class="status-{{$order['order_status']}} class-all">
                            ...
                            <td>
                                @if ($parcel_order)
                                    <label class="badge badge-soft-primary">{{Str::limit($order->parcel_category?$order->parcel_category->name:__('messages.not_found'),20,'...')}}</label>
                                @elseif ($order->store)
                                    <label class="badge badge-soft-primary"><a href="{{route('admin.vendor.view', $order->store_id)}}" alt="view store">{{Str::limit($order->store?$order->store->name:__('messages.store deleted!'),20,'...')}}</a></label>
                                @else
                                    <label class="badge badge-soft-danger">{{Str::limit(__('messages.not_found'),20,'...')}}</label>
                                @endif
                            </td>
                            <td>
        ...
        <div id="datatableFilterSidebar" class="hs-unfold-content_ sidebar sidebar-bordered sidebar-box-shadow" style="display: none">
            <div class="card card-lg sidebar-card sidebar-footer-fixed">
                ...
                <form class="card-body sidebar-body sidebar-scrollbar" action="{{route('admin.order.filter')}}" method="POST" id="order_filter_form">
                    ...
                    @if (!$parcel_order)
                        <hr class="my-4">
                        <small class="text-cap mb-3">{{__('messages.store')}}</small>
                        <div class="mb-2" style="border: 1px solid #8080803d;border-radius: 5px">
...
@push('script_2')
            ...
            var datatable = $.HSCore.components.HSDatatables.init($('#datatable'), {
                ...
                buttons: [
                    ...
                    },
                    {
                        ...
                        {
                            window.location.href = '{{route("admin.order.export",['status'=>$status,'file_type'=>'excel','type'=>$parcel_order?'parcel':'order'])}}';
                        }
                    },
                    {
                        extend: 'csv',
    ...
    <script>
        $('#search-form').on('submit', function () {
            ...
            $.get({
                url: '{{$parcel_order?url()->full() : route('admin.order.search')}}',
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
