                    ...
                    <div class="card h-100">
                        <div class="card-header">
                            ...
                            <button type="button" class="btn-sm btn-primary" data-toggle="modal" data-target="#updatesettingsmodal">
                                {{$store->discount?__('messages.update').' '.__('messages.discount'):__('messages.add').' '.__('messages.discount')}}
                            </button>
                        </div>
                        <div class="card-body">
                            @if($store->discount)
                            <div class="row">
                                <div class="col-6 border-right">
                                    <ul class="list-unstyled list-unstyled-py-3 text-dark mb-3">
                                        <li class="pb-1 pt-1">{{__('messages.start')}} {{__('messages.date')}} : {{$store->discount?date('Y-m-d',strtotime($store->discount->start_date)):''}}</li>
                                        <li class="pb-1 pt-1">{{__('messages.end')}} {{__('messages.date')}} : {{$store->discount?date('Y-m-d', strtotime($store->discount->end_date)):''}}</li>
                                        <li class="pb-1 pt-1">{{__('messages.start')}} {{__('messages.time')}} : {{$store->discount?date(config('timeformat'), strtotime($store->discount->start_time)):''}}</li>
                                        <li class="pb-1 pt-1">{{__('messages.end')}} {{__('messages.time')}} : {{$store->discount?date(config('timeformat'), strtotime($store->discount->end_time)):''}}</li>
                                    </ul>
                                </div>
                                <div class="col-6">
                                    <ul class="list-unstyled list-unstyled-py-3 text-dark mb-3">
                                        <li class="pb-1 pt-1">{{__('messages.discount')}} : {{$store->discount?round($store->discount->discount):0}}%</li>
                                        <li class="pb-1 pt-1">{{__('messages.max')}} {{__('messages.discount')}} : {{\App\CentralLogics\Helpers::format_currency($store->discount?$store->discount->max_discount:0)}}</li>
                                        <li class="pb-1 pt-1">{{__('messages.min')}} {{__('messages.purchase')}} : {{\App\CentralLogics\Helpers::format_currency($store->discount?$store->discount->min_purchase:0)}}</li>
                                    </ul>
                                </div>
                            </div>
                            @else
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalCenterTitle">{{$store->discount?__('messages.update'):__('messages.add')}} {{__('messages.discount')}}</h5>
      ...
      <div class="modal-body">
        <form action="{{route('admin.vendor.discount',[$store['id']])}}" method="post" id="discount-form">
            ...
            <div class="form-group text-center">
                <button type="submit" class="btn btn-primary">{{$store->discount?__('messages.update'):__('messages.add')}}</button>
                @if($store->discount)
                <button type="button" onclick="form_alert('discount-{{$store->id}}','Want to remove discount?')" class="btn btn-danger">{{__('messages.delete')}}</button>
                @endif
            </div>
        </form>
